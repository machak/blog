...
<article>
  ...
  <section class="share">
    <p>
      <a class="a2a_dd" href="http://www.addtoany.com/share_save"><img src="http://static.addtoany.com/buttons/share_save_120_16.gif" width="120" height="16" alt="Share/Bookmark"/></a>
    </p>
  </section>
  ...
  <c:if test="${not empty comments and fn:length(comments)>0}">
    <section class="comments">
      <h2>${fn:length(comments)}
        <c:choose><c:when test="${fn:length(comments) eq 1}">comment</c:when><c:otherwise>comments</c:otherwise></c:choose></h2>
      <c:forEach var="comment" items="${comments}">
        <div class="comment" id="${comment.name}">
          <c:choose>
            <c:when test="${not empty comment.website}"><h3>
              <a href="${comment.website}" rel="external">${comment.person}</a> said</h3></c:when>
          ...
          <p>
            <time datetime="${commentdate}">${comment.printableDate}</time>
          </p>
        </div>
      </c:forEach>
    </section>
    ...
    <section>
      ...
      <form method="post" action="${addURL}" class="commentform" id="commentform">
        <fieldset>
          ...
          <ul>
            <li><label for="person">Name: *</label><input type="text" id="person" name="person" required="required"/>
            </li>
            ...
            <li>
              <label for="comment">Comment: *</label><textarea name="comment" id="comment" rows="4" cols="40" required="required"></textarea>
            </li>
